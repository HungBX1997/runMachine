...
/*Call start function when the open character device*/
...
static const struct watchdog_ops imx2_wdt_ops = {
	.owner = THIS_MODULE,
	.start = imx2_wdt_start,  /* when open character device */
	.ping = imx2_wdt_ping,    /* to set reload value*/
	.set_timeout = imx2_wdt_set_timeout,  /*daemen set ioctl*/
...
static int __init imx2_wdt_probe(struct platform_device *pdev)
{
	...
	int ret;
	u32 val;
	/*Alloc memory for wdev*/
	wdev = devm_kzalloc(&pdev->dev, sizeof(*wdev), GFP_KERNEL);
	/*Check alloc*/
	if (!wdev)
		return -ENOMEM;
	/*get real address from device tree*/
	res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	/*Map virtual address for real address*/
	base = devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR(base))
		return PTR_ERR(base);
	/*Assign base for object*/
	...
	wdog			= &wdev->wdog;
	wdog->info		= &imx2_wdt_info; /*Information of oject*/
	wdog->ops		= &imx2_wdt_ops;  /*Functions are used in character device*/
	wdog->min_timeout	= 1;
	...
	if (ret)
		return ret;
	/*Read value of register and assign into val*/
	...
	ret = watchdog_register_device(wdog);   /*Register*/
	if (ret) {
...
static int __exit imx2_wdt_remove(struct platform_device *pdev)
{
	...
	watchdog_unregister_device(wdog);    /*Unregister*/
