...
/**/
static ssize_t imx2_wdt_read(struct file *filep, char __user *buf, size_t len, loff_t *offset);

static void my_timer_callback(unsigned long data)
{
	/*Set up new timer for next jump*/
	countTimer++;
	remainTimeout = *(unsigned long *)data - countTimer;
	buf_WR[0] = remainTimeout % 10 + '0';
	buf_WR[1] = remainTimeout / 10 + '0';
	mod_timer(&my_timer, jiffies + (1 * HZ));
}

static int config_Timer(int timeOut)
{
...
static inline void imx2_wdt_setup(void)
{
	...
	pr_info("Time out is: %d\n", (int)imx2_wdt.timeout);
	/* enable the watchdog */
	val |= IMX2_WDT_WCR_WDE;
	__raw_writew(val, imx2_wdt.base + IMX2_WDT_WCR);
}
...
static void imx2_wdt_start(void)
{
	if (!test_and_set_bit(IMX2_WDT_STATUS_STARTED, &imx2_wdt.status)) {
		...
		imx2_wdt_setup();
		config_Timer((int)imx2_wdt.timeout);
